fn main() {
    ...
    let mut lines = input
        ...
        .collect::<Vec<Vec<i64>>>();
    let mut next = lines.pop().unwrap();
    let mut next2 = lines.pop().unwrap();
    println!("oof: {:?} + {:?}", pp(&next), pp(&next2));
    let mut res = add(&mut next, &mut next2);
    loop {
        if lines.is_empty() {
            break;
        }
        let mut next2 = lines.pop().unwrap();
        println!("oof: {:?} + {:?}", pp(&res), pp(&next2));
        res = add(&mut res, &mut next2);
    }
    println!("done: {:?}", pp(&res));
    println!("product: {:?}", product(&res));
}

fn add(l: &mut Vec<i64>, r: &mut Vec<i64>) -> Vec<i64> {
